const DIAGRAM_CONTAINER: &str = "#main > div.docblock.type-decl > div > div > div.railroad_container";
...
impl Browser {
    fn extract_extension() -> Result<tempdir::TempDir, failure::Error>  {
        let packed_path = std::env::var_os("MACRO_RAILROAD_PACKED").expect("Archive path not given by env");
    ...
    fn navigate_to_macro_page(&self, url: &str) -> Result<std::sync::Arc<headless_chrome::browser::tab::Tab>, failure::Error> {
    ...
    #[cfg(test)]
    fn testable_tab(&self) -> Result<std::sync::Arc<headless_chrome::browser::tab::Tab>, failure::Error> {
...
fn main() -> Result<(), failure::Error> {
    ...
    let screenshot = |url: &str, fname: &str| -> Result<(), failure::Error> {
        ...
        let png_data = tab.capture_screenshot(headless_chrome::protocol::page::ScreenshotFormat::PNG, None, true)?;
        std::fs::write(fname, &png_data)?;
        Ok(())
    };

    screenshot(URL_PANIC, "std_panic.png")?;
    screenshot("https://docs.rs/nom/4.2.2/nom/macro.named_attr.html", "nom_named_attr.png")?;
    Ok(())
}
...
mod tests {
    ...
    const LEGEND: &str = "#main > div.docblock.type-decl > div > div > div.railroad_container > svg > g > g.legend";
    const OPTIONS: &str = "#main > div.docblock.type-decl > div > div > div.railroad_container > div > div > img";
    ...
    fn placement() -> Result<(), failure::Error> {
        ...
        assert!(inline_dia_box.content.within_horizontal_bounds_of(&macro_block_box.margin));
        Ok(())
    }
